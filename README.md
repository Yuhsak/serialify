...
Sometimes it cloud be dangerous to deserializing a Function object because `eval()` will be called at the time.
...
When calling `deserialize()` , or `parse()`, an option can be set with value of `{dangerouslyDeserializeFunction: boolean}`.

If the value is set to `true`, Function value won't be deserialized and will be omitted from output deserialized object. {default: `false`}
